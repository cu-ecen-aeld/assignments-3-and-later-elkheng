...
cp ${SYSROOT}/$(${CROSS_COMPILE}readelf -a bin/busybox | grep "program interpreter" | awk -F': ' '{print $2} ' | tr -d ']') ${OUTDIR}/rootfs/lib
for lib in $(${CROSS_COMPILE}readelf -a bin/busybox | grep "Shared library" | awk -F': ' '{print $2}' | tr -d '[]'); do
...
sudo chown -R $(whoami) ${FINDER_APP_DIR}
